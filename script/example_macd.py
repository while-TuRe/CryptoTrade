        ...
        if not self.position:
            if self.params.user_scope in [btc_enum.Scope.BUY,btc_enum.Scope.ALL]:
                ...
                if self.sma_short[0] < self.sma_short[-1] \
                    and self.sma_middle[0] < self.sma_middle[-1] \
                    and self.sma_long[0] < self.sma_long[-1] \
                    and (self.dataopen[0]-self.dataclose[0])> self.average_range()*3 \
                    and self.volume[0] > self.average_volume()*1.8 :
...
if __name__ == '__main__':
    ...
    pf.create_full_tear_sheet(   
        ...
        live_start_date='2023-12-01', # 指定日期    
        round_trips=True)   
    # 当前节点的图表将展现
    # cerebro.plot()
